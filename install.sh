...
try:
    ...
    def hash_pw(p): return _bcrypt.hashpw(p.encode()[:72], _bcrypt.gensalt(12)).decode()
    def check_pw(p, h): return _bcrypt.checkpw(p.encode()[:72], h.encode())
    ...
    row = db.query(User).filter(User.username == "${ADMIN_USER}").first()
    assert check_pw("${ADMIN_PASS}", row.password_hash), "verify failed!"
...
INITPY
    ...
    if [[ $? -ne 0 ]]; then
        print_warn "DB init error — see log"
    else
