...
preflight() {
    ...
    # ── Умный выбор портов: если 443/80 заняты (например 3x-ui) — берём другие ──
    _port_free() {
        ! ss -tlnp 2>/dev/null | grep -qE ":${1}[[:space:]]|:${1}$"
    }

    # HTTPS порт (443 → 8443 → 8444 → 8445 → 9443)
    if ! _port_free "$PANEL_PORT"; then
        print_warn "Порт ${PANEL_PORT} занят (3x-ui?). Ищем свободный HTTPS порт..."
    ...
    # HTTP порт (80 → 8080 → 8081 → 8088 → 9080)
    if ! _port_free "$HTTP_PORT"; then
        print_warn "Порт ${HTTP_PORT} занят (3x-ui?). Ищем свободный HTTP порт..."
    ...
    # Бэкенд порт 8000 — если занят, берём следующий свободный
    BACKEND_PORT=8000
