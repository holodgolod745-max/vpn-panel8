...
start_spinner() {
    ...
    SPINNER_PID=$!
    # Не используем disown — он конфликтует с set -e в некоторых bash версиях
}
...
# ─────────────────────────────────────────────
setup_ssl() {
    print_step "SSL сертификат (самоподписанный)"

    run_step "Генерация RSA-4096 (10 лет)" bash -c "
        openssl req -x509 -newkey rsa:4096 -nodes -days 3650 \
            -keyout '${INSTALL_DIR}/certs/key.pem' \
            -out    '${INSTALL_DIR}/certs/cert.pem' \
            -subj   '/CN=${SERVER_IP}/O=FEDUK/OU=Proxy/C=RU' \
...
.logo-icon { font-size:26px; margin-bottom:4px; }
.logo-title { font-size:20px; font-weight:900; background:var(--grad); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
.logo-sub { font-size:11px; color:var(--txt2); font-weight:500; margin-top:2px; }
...
.modal { background:var(--bg2); border:1px solid var(--border); border-radius:var(--r); padding:30px; width:100%; max-width:480px; box-shadow:var(--shadow); animation:popIn .22s cubic-bezier(.34,1.56,.64,1); }
...
.fi, .fs {
  ...
  padding:10px 14px; color:var(--txt); font-size:14px; outline:none;
...
.login-wrap {
  ...
  background-image:
    radial-gradient(ellipse at 20% 50%,rgba(59,130,246,.1) 0%,transparent 60%),
    radial-gradient(ellipse at 80% 20%,rgba(139,92,246,.1) 0%,transparent 60%);
}
...
<div id="pg-app" class="layout" style="display:none">
  <nav class="sidebar">
    <div class="logo-area">
      ...
      <div class="logo-title">FEDUK Panel</div>
      <div class="logo-sub">Proxy v3.1</div>
    </div>
    <div class="nav">
      ...
      <div class="nav-item" data-page="inbounds">
        <span class="ico">🔌</span>
        <span>Inbounds</span>
        <span class="nav-badge" id="nb-inb">0</span>
      </div>
      <div class="nav-item" data-page="clients">
        <span class="ico">👥</span>
        <span>Клиенты</span>
        <span class="nav-badge" id="nb-cli">0</span>
      </div>
  ...
  <main class="content">
    ...
    <div id="pg-inbounds" style="display:none">
      ...
      <div class="tbl-wrap">
        <div class="tbl-hdr">
          <div>
            <div class="tbl-title">📡 Список inbounds</div>
            <div class="tbl-sub">Управление прокси каналами</div>
          </div>
        </div>
    ...
    <div id="pg-clients" style="display:none">
      ...
      <div class="tbl-wrap">
        <div class="tbl-hdr">
          <div>
            <div class="tbl-title">👥 Список клиентов</div>
            <div class="tbl-sub">Управление пользователями</div>
          </div>
        </div>
        <div class="tbl-scroll">
          <table>
    ...
    <div id="pg-status" style="display:none">
      ...
      <div class="tbl-wrap">
        <div class="tbl-hdr">
          <div class="tbl-title">🔧 Системные сервисы</div>
        </div>
        <table>
          <thead><tr><th>Сервис</th><th>Описание</th><th>Статус</th></tr></thead>
          <tbody id="tb-status">
            <tr class="loading-row"><td colspan="3">Загрузка...</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
<div class="modal-ov" id="mo-client">
  <div class="modal">
    ...
    <div class="fg">
      <label class="fl">Лимит трафика (GB, 0 = безлимит)</label>
      <input type="number" id="cli-limit" class="fi" value="0" min="0" step="1">
    </div>
    <div class="fg">
      <label class="fl">Срок действия (необязательно)</label>
...
function showApp() {
  ...
  dashTimer = setInterval(() => {
    if (curPage === "dashboard" && token) loadDashboard();
  }, 15000);
}
...
function navigate(page) {
  ["dashboard", "inbounds", "clients", "status"].forEach(p => {
...
async function loadDashboard() {
  try {
    const d = await apiFetch("GET", "/dashboard");
    if (!d) return;
    const s = d.system, p = d.proxy;

    const cpu = s.cpu_percent.toFixed(1);
    $("d-cpu").textContent = cpu + "%";
    ...
    toast("Ошибка дашборда: " + e.message, "error");
  }
}
...
async function loadInbounds() {
  ...
  try {
    ...
    const PROTO_COLOR = { vmess:"b-blue", vless:"b-cyan", trojan:"b-purple", shadowsocks:"b-yellow" };
    tb.innerHTML = inbList.map((i, idx) => `<tr>
      <td>${idx + 1}</td>
      <td><strong>${esc(i.remark)}</strong><br><span style="font-size:11px;color:var(--txt2);font-family:var(--mono)">${esc(i.tag)}</span></td>
      <td><span class="badge ${PROTO_COLOR[i.protocol] || 'b-blue'}">${esc(i.protocol.toUpperCase())}</span></td>
      <td><strong>${i.port}</strong></td>
      <td>${i.clients_count || 0}</td>
      <td>
        <span class="badge ${i.enabled ? 'b-green' : 'b-red'}">
          ${i.enabled ? '● Активен' : '● Выкл'}
        </span>
      </td>
...
async function createInbound() {
  ...
  if (!port || port < 1 || port > 65535) { toast("Введите корректный порт (1–65535)", "error"); return; }

  const settingsMap = {
    vless:       { clients: [], decryption: "none" },
    vmess:       { clients: [] },
    trojan:      { clients: [] },
    shadowsocks: { method: "chacha20-ietf-poly1305", password: rndStr(16), network: "tcp" },
  };
  try {
    await apiFetch("POST", "/inbounds", { remark, protocol: proto, port, settings: settingsMap[proto] || {} });
    closeModal("mo-inbound");
    $("inb-remark").value = "";
    $("inb-port").value = "";
    toast("Inbound создан!", "success");
    loadInbounds();
  } catch (e) {
    toast(e.message, "error");
  }
...
async function toggleInbound(id) {
  try {
    await apiFetch("PATCH", `/inbounds/${id}/toggle`);
    loadInbounds();
  } catch (e) {
    toast(e.message, "error");
  }
...
async function delInbound(id) {
  ...
  try {
    await apiFetch("DELETE", "/inbounds/" + id);
    toast("Inbound удалён", "success");
    loadInbounds();
  } catch (e) {
    toast(e.message, "error");
  }
  ...
  try {
    ...
    tb.innerHTML = clients.map((c, idx) => {
      ...
      return `<tr>
        ...
        <td>
          <span class="mono" onclick="cp('${esc(c.uuid)}')" title="Копировать UUID">${c.uuid.substring(0,13)}…</span>
        </td>
        <td>${inb.tag ? `<span class="badge b-blue">${esc(inb.tag)}</span>` : `#${c.inbound_id}`}</td>
        <td>${fmtB(total)}${limStr}</td>
...
async function openClientModal() {
  ...
  if (!inbList.length) {
    toast("Сначала создайте inbound", "error");
    return;
  }
  sel.innerHTML = inbList.map(i =>
    `<option value="${i.id}">${esc(i.remark)} — ${esc(i.protocol.toUpperCase())} :${i.port}</option>`
  ).join("");
  openModal("mo-client");
}
...
async function createClient() {
  ...
  try {
    await apiFetch("POST", "/clients", {
      email, inbound_id: inb_id,
      traffic_limit: limit,
      expire_date: expire,
    });
...
async function loadStatus() {
  try {
    const d = await apiFetch("GET", "/status");
    if (!d) return;
    const DESC = {
      "feduk":        "FastAPI панель управления",
      "xray-feduk":   "Xray-core прокси сервер",
      "nginx":        "Обратный прокси / HTTPS",
      "redis-server": "Кэш и сессии",
    };
...
// ── PROTO HINT ───────────────────────────────────────────────
const HINTS = {
  vmess:       "VMess — стандартный протокол с шифрованием, широкая совместимость",
  vless:       "VLESS — облегчённый протокол, меньше накладных расходов на CPU",
  trojan:      "Trojan — маскируется под легитимный HTTPS трафик",
  shadowsocks: "Shadowsocks — простой и быстрый протокол шифрования",
};
function updateProtoHint() {
  $("proto-hint").textContent = HINTS[$("inb-proto").value] || "";
}
...
function cp(text) {
  navigator.clipboard.writeText(text)
    .then(() => toast("UUID скопирован!", "success"))
...
show_summary() {
    ...
    echo -e "  ${YELLOW}⚠  Браузер предупредит о сертификате — нажмите «Продолжить»${R}"
    echo -e "  ${DIM}   (самоподписанный сертификат — это нормально)${R}"
    echo
}
