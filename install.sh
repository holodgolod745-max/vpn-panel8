...
interactive_setup() {
    ...
    # Reject IP addresses in domain field
...
setup_ssl() {
    ...
    # Guard: Let's Encrypt does not work with raw IP addresses
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ "$DOMAIN" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        print_warn "Domain is an IP — switching to self-signed certificate"
        USE_LETSENCRYPT=false
        DOMAIN=""
    fi
...
configure_nginx() {
    ...
    # Detect nginx version to choose correct http2 syntax:
    # < 1.25.1 : listen PORT ssl http2;
    # >= 1.25.1: listen PORT ssl; http2 on;
    local nginx_major nginx_minor nginx_patch
    nginx_major=$(nginx -v 2>&1 | grep -oP '\d+\.\d+\.\d+' | cut -d. -f1)
    nginx_minor=$(nginx -v 2>&1 | grep -oP '\d+\.\d+\.\d+' | cut -d. -f2)
    nginx_patch=$(nginx -v 2>&1 | grep -oP '\d+\.\d+\.\d+' | cut -d. -f3)
