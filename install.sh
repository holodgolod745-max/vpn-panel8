...
# ─────────────────────────────────────────────
interactive_setup() {
    # set -e causes script death when read gets empty Enter (returns 1)
    set +euo pipefail
    local OLD_IFS="$IFS"
    IFS=$' \t\n'
...
setup_ssl() {
    ...
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ -n "$DOMAIN" ]]; then
        ...
        # Must stop nginx — certbot --standalone needs port 80
        ...
        if certbot certonly --standalone --non-interactive --agree-tos \
            ...
            log_info "Self-signed fallback cert"
        fi
...
configure_nginx() {
    ...
    log_info "Nginx ${ngx_ver}: using listen '${http2_listen}' http2_line='${http2_line}'"
