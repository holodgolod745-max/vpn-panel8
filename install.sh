...
CURRENT_STEP=0
TOTAL_STEPS=16
SAVED_DATA_PATH=""  # путь к бэкапу данных при переустановке
...
die() {
    ...
    if [[ -n "${SAVED_DATA_PATH:-}" ]] && [[ -d "$SAVED_DATA_PATH" ]]; then
        echo -e "${YELLOW}Ваши данные сохранены в: ${SAVED_DATA_PATH}${R}"
    fi
    echo
    exit 1
}
...
preflight() {
    ...
    if [[ "$ram_mb" -lt 400 ]]; then
        die "Недостаточно RAM: ${ram_mb}MB. Минимум 512MB."
    fi
    print_ok "RAM: ${ram_mb}MB — OK"

    local disk_mb
    disk_mb=$(df / --output=avail -m 2>/dev/null | tail -1 | tr -d ' ')
    if [[ "${disk_mb:-0}" -lt 1800 ]]; then
        die "Недостаточно места на диске: ${disk_mb:-?}MB. Минимум 2GB."
    fi
    print_ok "Диск: свободно ${disk_mb}MB — OK"
    ...
    # ── Обнаружение других панелей ────────────────────────────
    local conflict_found=false
    for panel_svc in x-ui xui marzban 3x-ui; do
        if systemctl is-active --quiet "$panel_svc" 2>/dev/null; then
            print_warn "Обнаружена другая панель: ${panel_svc} — возможны конфликты портов"
            conflict_found=true
        fi
    done
    if [[ -d /etc/x-ui ]] || [[ -d /usr/local/x-ui ]]; then
        print_warn "Найдены файлы x-ui — порты будут выбраны автоматически"
        conflict_found=true
    fi
    [[ "$conflict_found" == true ]] && print_warn "Конфликтующие панели найдены — порты будут подобраны автоматически"

    # ── Повторная установка: сохранение данных ────────────────
    if [[ -f "${CRED_FILE}" ]] || [[ -d "${INSTALL_DIR}" ]]; then
        print_warn "Обнаружена предыдущая установка FEDUK Panel"
        set +e
        local save_data="y"
        if [[ -t 0 ]] || [[ -c /dev/tty ]]; then
            read -r -t 30 -p "  → Сохранить БД и сертификаты? [Y/n]: " save_data </dev/tty || save_data="y"
        fi
        set -e
        ...
        if [[ "${save_data,,}" != "n" ]]; then
            ...
            local backup_path="/root/feduk_backup_${ts}"
            mkdir -p "$backup_path"
            [[ -d "${INSTALL_DIR}/data" ]]  && cp -r "${INSTALL_DIR}/data"  "$backup_path/" 2>/dev/null || true
            [[ -d "${INSTALL_DIR}/certs" ]] && cp -r "${INSTALL_DIR}/certs" "$backup_path/" 2>/dev/null || true
            [[ -f "${CONFIG_DIR}/config.yml" ]] && cp "${CONFIG_DIR}/config.yml" "$backup_path/" 2>/dev/null || true
            print_ok "Данные сохранены: ${backup_path}"
            SAVED_DATA_PATH="$backup_path"
        else
            SAVED_DATA_PATH=""
            print_warn "Данные НЕ сохраняются — продолжаем чистую установку"
        fi
    fi
    ...
    # ── Проверка DNS перед Let's Encrypt ─────────────────────
    if [[ -n "$DOMAIN" ]]; then
        print_info "Проверка DNS для домена ${DOMAIN}..."
        local dns_ip=""
        dns_ip=$(dig +short "$DOMAIN" 2>/dev/null | grep -E '^[0-9]+\.' | head -1) || true
        if [[ -z "$dns_ip" ]]; then
            print_warn "DNS для ${DOMAIN} не резолвится — используем self-signed"
            DOMAIN=""
        elif [[ "$dns_ip" != "$SERVER_IP" ]]; then
            print_warn "DNS ${DOMAIN} → ${dns_ip}, но IP сервера ${SERVER_IP} — используем self-signed"
            DOMAIN=""
        else
            USE_LETSENCRYPT=true
            print_ok "DNS OK: ${DOMAIN} → ${dns_ip}"
        fi
    fi
    ...
    if ! _port_free "$PANEL_PORT"; then
        print_warn "Порт ${PANEL_PORT} занят. Ищем свободный HTTPS порт..."
        for _p in 8443 8444 8445 9443 10443; do
            if _port_free "$_p"; then
                PANEL_PORT="$_p"
                print_ok "HTTPS порт → ${PANEL_PORT}"
                break
            fi
        done
    else
        print_ok "HTTPS порт ${PANEL_PORT} — свободен"
    fi

    if ! _port_free "$HTTP_PORT"; then
        print_warn "Порт ${HTTP_PORT} занят. Ищем свободный HTTP порт..."
        for _p in 8080 8081 8088 9080; do
            if _port_free "$_p"; then
                HTTP_PORT="$_p"
                print_ok "HTTP порт → ${HTTP_PORT}"
                break
            fi
        done
    else
        print_ok "HTTP порт ${HTTP_PORT} — свободен"
    fi

    BACKEND_PORT=8000
    for _p in 8000 8001 8002 8003 8010; do
        if _port_free "$_p"; then
            BACKEND_PORT="$_p"
            break
        fi
    done
    [[ "$BACKEND_PORT" != "8000" ]] && print_warn "Порт 8000 занят → бэкенд на ${BACKEND_PORT}"

    log_info "Preflight OK. IP=${SERVER_IP} ARCH=${XRAY_ARCH} HTTPS=${PANEL_PORT} HTTP=${HTTP_PORT} BACKEND=${BACKEND_PORT}"
    progress_bar
}
...
install_packages() {
    ...
    # Снимаем зависшие lock-файлы APT (причина кода 100)
    ...
    # Пробуем обновить APT до 3 раз с паузой
    local apt_ok=false
    for _try in 1 2 3; do
        ...
        log_warn "apt-get update попытка ${_try} неудачна, ждём 5 сек..."
        sleep 5
    done
    if [[ "$apt_ok" == true ]]; then
        print_ok "Обновление APT"
    else
        print_warn "APT update завершился с ошибкой, продолжаем (могут быть устаревшие пакеты)"
        log_warn "apt-get update failed after 3 attempts — continuing anyway"
    fi
    ...
    run_step "Установка зависимостей" \
        apt-get install -y -qq \
            ...
            jq logrotate cron net-tools dnsutils
...
configure_firewall() {
    ...
    # ── Оптимизация ядра: BBR + TCP + лимиты ──────────────────
    print_info "Применяем оптимизацию сети..."
    ...
    cat > /etc/sysctl.d/99-feduk.conf << 'SYSEOF'
# FEDUK Panel — kernel optimizations
# TCP BBR
...
SYSEOF
    sysctl -p /etc/sysctl.d/99-feduk.conf >> "$LOG_FILE" 2>&1 || true

    # Лимиты системы
    cat > /etc/security/limits.d/99-feduk.conf << 'LIMEOF'
* soft nofile 1048576
* hard nofile 1048576
...
LIMEOF
    ...
    if [[ "$bbr_ok" == true ]]; then
        print_ok "BBR включён + TCP оптимизация применена"
    else
        print_ok "TCP оптимизация применена (BBR недоступен на этом ядре)"
    fi
...
from sqlalchemy import (
    ...
    Boolean, DateTime, Text, ForeignKey, func, event
)
...
def set_wal_mode(dbapi_conn, _):
    """WAL mode: чтения не блокируют запись"""
...
def _blacklist_token(token: str):
    """Добавить токен в blacklist при logout/смене пароля"""
    if _r and REDIS_OK:
        try:
            _r.setex(f"bl:{token}", TOKEN_TTL * 60, "1")
        except Exception:
            pass
...
def _is_blacklisted(token: str) -> bool:
    if _r and REDIS_OK:
        try:
            return bool(_r.exists(f"bl:{token}"))
        except Exception:
            pass
    return False
...
def build_xray_config(db: Session) -> dict:
    ...
    for inb in db.query(Inbound).filter(Inbound.enabled == True).all():
        ...
        for c in inb.clients:
            ...
            if c.traffic_limit > 0 and (c.traffic_up + c.traffic_down) >= c.traffic_limit:
                continue
...
async def logout(token: str = Depends(oauth2), _=Depends(get_current_user)):
...
async def dashboard(
    ...
    # Клиенты истекающие через 7 дней
    week_later = datetime.utcnow() + timedelta(days=7)
    expiring_soon = db.query(func.count(Client.id)).filter(
        Client.expire_date != None,
        Client.expire_date <= week_later,
        Client.expire_date >= datetime.utcnow(),
        Client.enabled == True
    ).scalar() or 0
    ...
    top_clients = db.query(
        Client.email,
        (Client.traffic_up + Client.traffic_down).label("total")
    ).order_by((Client.traffic_up + Client.traffic_down).desc()).limit(10).all()
    ...
    return {
        ...
        "proxy": {
            ...
            "top_clients":     [{"email": e, "total": t} for e, t in top_clients],
        },
    }
...
@app.post("/api/clients/bulk", tags=["Clients"])
async def bulk_clients(
    ...
    """Массовые действия: enable / disable / delete"""
    clients = db.query(Client).filter(Client.id.in_(data.ids)).all()
    if not clients:
        raise HTTPException(404, detail="Клиенты не найдены")

    if data.action == "enable":
        for c in clients: c.enabled = True
        db.commit()
    elif data.action == "disable":
        for c in clients: c.enabled = False
        db.commit()
    elif data.action == "delete":
        for c in clients: db.delete(c)
        db.commit()
    else:
        raise HTTPException(400, detail="Неизвестное действие")

    bg.add_task(xray_apply, db)
    return {"ok": True, "affected": len(clients)}
...
@app.get("/api/version-check", tags=["System"])
async def version_check(_=Depends(get_current_user)):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            return {"current": "3.1.0", "latest_xray": latest, "ok": True}
    except Exception:
        return {"current": "3.1.0", "latest_xray": None, "ok": False}
...
async def change_password(
    ...
    db.commit()
    # Инвалидируем текущий токен после смены пароля
    _blacklist_token(token)
    return {"ok": True}
...
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect width='32' height='32' rx='8' fill='%233b82f6'/><text y='22' x='5' font-size='20'>🛡</text></svg>">
...
.splash-logo { font-size:56px; margin-bottom:16px; animation:splashPulse 1.2s ease-in-out infinite; }
.splash-title { font-size:24px; font-weight:900; background:var(--grad); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; margin-bottom:32px; }
.splash-bar { width:200px; height:3px; background:var(--bg4); border-radius:4px; overflow:hidden; }
.splash-fill { height:100%; background:var(--grad); border-radius:4px; animation:splashLoad 1.5s ease-in-out forwards; }
...
/* ── Status dot ───────────────────────────────── */
#status-dot { width:8px; height:8px; border-radius:50%; background:var(--ok); display:inline-block; margin-right:6px; box-shadow:0 0 6px var(--ok); transition:background .3s,box-shadow .3s; }
...
#api-status-bar { display:flex; align-items:center; font-size:11px; color:var(--txt2); padding:8px 14px; }
...
@media(max-width:768px){
  ...
  .sidebar { width:100%; min-width:unset; height:auto; flex-direction:row; flex-wrap:wrap; border-right:none; border-bottom:1px solid var(--border); }
  .logo-area { padding:12px 14px; border-bottom:none; flex:1; }
  .nav { flex-direction:row; flex-wrap:wrap; padding:4px 6px; gap:2px; overflow-x:auto; flex:1; }
  ...
  .td-actions { flex-direction:column; gap:3px; }
  .modal-box { width:95vw !important; margin:10px; }
...
/* ── Copy badge ───────────────────────────────── */
.copy-wrap { display:inline-flex; align-items:center; gap:5px; cursor:pointer; }
.copy-icon { font-size:12px; opacity:0.5; transition:opacity .15s; }
.copy-wrap:hover .copy-icon { opacity:1; }

/* ── Confirm modal ────────────────────────────── */
#mo-confirm .modal-box { max-width:400px; }
.confirm-msg { font-size:15px; margin:8px 0 20px; color:var(--txt); }
.confirm-name { font-weight:700; color:var(--err); }

/* ── Traffic progress bar ─────────────────────── */
.traf-progress { height:6px; background:var(--bg4); border-radius:4px; overflow:hidden; margin-top:4px; min-width:80px; }
.traf-progress-fill { height:100%; border-radius:4px; transition:width .3s; }
.traf-pct-low  { background:var(--ok); }
.traf-pct-mid  { background:var(--warn); }
.traf-pct-high { background:var(--err); }

/* ── Expiry badge ─────────────────────────────── */
.expiry-badge { display:inline-flex; align-items:center; gap:5px; background:rgba(245,158,11,.15); color:var(--warn); border:1px solid rgba(245,158,11,.3); border-radius:20px; padding:2px 10px; font-size:11px; font-weight:700; }

/* ── Bulk actions bar ─────────────────────────── */
#bulk-bar { display:none; align-items:center; gap:10px; padding:10px 16px; background:rgba(59,130,246,.1); border:1px solid rgba(59,130,246,.2); border-radius:var(--r2); margin-bottom:12px; }
#bulk-bar.show { display:flex; }
#bulk-count { font-size:13px; color:var(--acc1); font-weight:600; }
...
<div id="splash">
  ...
  <div class="splash-title">FEDUK Panel</div>
...
<!-- CONFIRM MODAL -->
<div id="mo-confirm" class="modal" style="display:none">
  <div class="modal-box" style="max-width:400px">
    <div class="modal-head"><span id="confirm-title">Подтверждение</span></div>
    <div class="modal-body">
      <p class="confirm-msg" id="confirm-msg">Вы уверены?</p>
      <div style="display:flex;gap:10px;justify-content:flex-end">
        <button class="btn btn-secondary" onclick="closeModal('mo-confirm')">Отмена</button>
        <button class="btn btn-danger" id="confirm-ok-btn">Удалить</button>
      </div>
    </div>
  </div>
</div>
...
<div id="pg-app" class="layout" style="display:none">
  <nav class="sidebar">
    ...
    <div class="sidebar-foot">
      <div id="api-status-bar">
        ...
        <span id="status-label">Подключено</span>
      </div>
  ...
  <main class="content">
    ...
    <div id="pg-dashboard" class="page">
      <div class="hdr">
        ...
        <div class="hdr-actions">
          <span id="expiry-badge" class="expiry-badge" style="display:none">⚠ <span id="expiry-count">0</span> истекает</span>
    ...
    <!-- CLIENTS -->
    <div id="pg-clients" class="page" style="display:none">
      <div class="hdr">
      ...
      <div id="bulk-bar">
        ...
        <button class="btn btn-secondary btn-sm" onclick="bulkAction('enable')">✅ Вкл</button>
        <button class="btn btn-secondary btn-sm" onclick="bulkAction('disable')">⏸ Выкл</button>
        <button class="btn btn-danger btn-sm" onclick="bulkAction('delete')">🗑 Удалить</button>
        <button class="btn btn-secondary btn-sm" style="margin-left:auto" onclick="selectedClients.clear();updateBulkBar();renderClients(cliList)">✕</button>
      </div>
      <div class="tbl-wrap">
        <div class="tbl-hdr">
        ...
        <div class="tbl-scroll">
          <table>
            <thead><tr>
              <th><input type="checkbox" id="check-all" onchange="toggleAllClients(this.checked)"></th>
              <th>#</th><th>Email</th><th>UUID</th><th>Inbound</th>
              <th>Трафик</th><th>Истекает</th><th>Статус</th><th>Действия</th>
            </tr></thead>
            <tbody id="tb-cli"><tr><td colspan="9" style="text-align:center;padding:40px">
              <div class="skel-row"><div class="skel-cell skeleton" style="width:30px"></div><div class="skel-cell skeleton" style="width:120px"></div><div class="skel-cell skeleton" style="width:200px"></div><div class="skel-cell skeleton" style="flex:1"></div></div>
              <div class="skel-row"><div class="skel-cell skeleton" style="width:30px"></div><div class="skel-cell skeleton" style="width:90px"></div><div class="skel-cell skeleton" style="width:200px"></div><div class="skel-cell skeleton" style="flex:1"></div></div>
            </td></tr></tbody>
          </table>
        </div>
      </div>
...
async function verifyTokenAndShow() {
  try {
    ...
    if (r.ok) { showApp(); } else { token=""; localStorage.removeItem("feduk_token"); $("pg-login").style.display="flex"; }
  } catch { $("pg-login").style.display="flex"; }
}
...
// Копировать с иконкой
function cpIcon(text) {
  return `<span class="copy-wrap" onclick="cp('${esc(text)}')" title="Копировать"><span>${esc(text)}</span><span class="copy-icon">📋</span></span>`;
}
function rndStr(n) {
...
function updatePageTitle() {
  ...
  document.title = active ? `FEDUK Panel (${active} активных)` : "FEDUK Panel";
}
...
async function pingStatus() {
  ...
  try {
    ...
    if (r.ok) {
      dot.className=""; dot.style.background="var(--ok)"; dot.style.boxShadow="0 0 6px var(--ok)";
      if(lbl) lbl.textContent="Подключено";
    } else throw new Error();
  } catch {
    dot.className="err"; dot.style.background="var(--err)"; dot.style.boxShadow="0 0 6px var(--err)";
...
function confirmAction(title, msg, onOk) {
  ...
  btn.onclick = () => { closeModal("mo-confirm"); onOk(); };
...
function showApp() {
  ...
  // Статус-пинг каждые 30 сек
  pingStatus();
  statusTimer=setInterval(pingStatus, 30000);
}
...
async function loadDashboard() {
  try {
    ...
    // Бейдж истекающих клиентов
    const exBadge=$("expiry-badge"), exCnt=$("expiry-count");
    if(exBadge&&exCnt&&p.expiring_soon>0){
      exCnt.textContent=p.expiring_soon;
      exBadge.style.display="inline-flex";
    } else if(exBadge) { exBadge.style.display="none"; }
...
async function loadInbounds() {
  const tb=$("tb-inb");
  tb.innerHTML='<tr><td colspan="9"><div class="skel-row"><div class="skel-cell skeleton" style="width:30px;height:12px"></div><div class="skel-cell skeleton" style="width:100px;height:12px"></div><div class="skel-cell skeleton" style="flex:1;height:12px"></div></div></td></tr>';
  try {
...
function renderInbounds(list) {
  ...
  tb.innerHTML=list.map((i,idx)=>`<tr>
    ...
    <td><div class="td-actions">
      ...
      <button class="btn btn-danger btn-sm" onclick="delInbound(${i.id},'${esc(i.remark)}')">🗑</button>
    </div></td>
  </tr>`).join("");
}
...
function delInbound(id, name) {
  confirmAction("Удалить Inbound", `Удалить <span class="confirm-name">${esc(name||"#"+id)}</span> и всех его клиентов?`, async()=>{
...
function renderClients(list) {
  const tb=$("tb-cli");
  if (!list.length) {
    tb.innerHTML=`<tr><td colspan="9"><div class="empty-state"><span class="ico">👥</span><p>Нет клиентов — добавьте первого!</p></div></td></tr>`;
    return;
  }
  ...
  tb.innerHTML=list.map((c,idx)=>{
    ...
    return `<tr>
      <td><input type="checkbox" ${checked} onchange="toggleClientSel(${c.id},this.checked)"></td>
      <td>${idx+1}</td>
      <td><strong>${esc(c.email)}</strong></td>
      <td><span class="mono copy-wrap" onclick="cp('${c.uuid}')" title="Копировать UUID">${c.uuid.substring(0,13)}… <span class="copy-icon">📋</span></span></td>
      <td>${inb.tag?`<span class="badge b-blue">${esc(inb.tag)}</span>`:`#${c.inbound_id}`}</td>
      <td>
        ${fmtB(total)}${limStr}
        ${c.traffic_limit?(()=>{const cls=pct<50?"traf-pct-low":pct<80?"traf-pct-mid":"traf-pct-high";return `<div class="traf-progress"><div class="traf-progress-fill ${cls}" style="width:${pct}%"></div></div>`;})():""}
      </td>
      ...
      <td><div class="td-actions">
        ...
        <button class="btn btn-danger btn-sm" onclick="confirmDelCli(${c.id},'${esc(c.email)}')">🗑</button>
      </div></td>
    </tr>`;
  }).join("");
}
...
function confirmDelCli(id, email) {
  confirmAction("Удалить клиента", `Удалить <span class="confirm-name">${esc(email)}</span>?`, ()=>delCli(id));
}
...
function toggleClientSel(id, checked) {
  if(checked) selectedClients.add(id); else selectedClients.delete(id);
  updateBulkBar();
}
function toggleAllClients(checked) {
  if(checked) cliList.forEach(c=>selectedClients.add(c.id));
...
async function bulkAction(action) {
  ...
  if(action==="delete"){
    confirmAction("Массовое удаление",`Удалить ${ids.length} клиентов?`, async()=>{
      try { await apiFetch("POST","/clients/bulk",{ids,action:"delete"}); selectedClients.clear(); updateBulkBar(); toast(`Удалено ${ids.length}","success`); loadClients(); }
      catch(e){ toast(e.message,"error"); }
    });
    return;
  }
  try {
    await apiFetch("POST","/clients/bulk",{ids,action});
    selectedClients.clear(); updateBulkBar();
    toast(`Готово: ${ids.length} клиентов`,"success"); loadClients();
  } catch(e){ toast(e.message,"error"); }
}
...
configure_nginx() {
    ...
    # Nginx worker оптимизация
    local cpu_count
    cpu_count=$(nproc 2>/dev/null || echo 1)
    cat > /etc/nginx/conf.d/feduk-perf.conf << PERFEOF
...
${http2_extra}
    ...
    # Auth с rate limiting — защита от брутфорса
    location /api/auth/ {
...
NGINXEOF
    ...
    # Проверяем конфиг — показываем причину если плохой
    if ! nginx -t >> "$LOG_FILE" 2>&1; then
        print_err "Nginx конфиг невалидный:"
        nginx -t 2>&1 | tail -5 | while IFS= read -r line; do echo "  $line"; done
        die "Nginx конфиг невалидный. Лог: ${LOG_FILE}"
    fi

    systemctl enable nginx >> "$LOG_FILE" 2>&1 || true

    # Снимаем процессы на портах перед запуском
    fuser -k "${HTTP_PORT}/tcp"  >> "$LOG_FILE" 2>&1 || true
    fuser -k "${PANEL_PORT}/tcp" >> "$LOG_FILE" 2>&1 || true
    sleep 1

    if systemctl restart nginx >> "$LOG_FILE" 2>&1; then
        print_ok "Nginx настроен (v${ngx_ver})"
    else
        print_warn "Nginx не запустился — повтор через 3 сек..."
        sleep 3
        if systemctl restart nginx >> "$LOG_FILE" 2>&1; then
            print_ok "Nginx настроен (v${ngx_ver})"
        else
            print_warn "Nginx не запустился: journalctl -u nginx для диагностики"
        fi
    fi
    progress_bar
}
...
ExecStartPre=${INSTALL_DIR}/xray/bin/xray -test -config ${INSTALL_DIR}/xray/configs/config.json
ExecStartPre=/bin/chmod 600 ${INSTALL_DIR}/certs/key.pem
...
ExecStartPre=/bin/bash -c 'for i in \$(seq 1 15); do redis-cli ping 2>/dev/null | grep -q PONG && exit 0; sleep 1; done; echo "Redis не готов"; exit 1'
ExecStartPre=/bin/bash -c 'chmod 750 ${INSTALL_DIR}/data; [ -f ${INSTALL_DIR}/data/config.db ] && chmod 640 ${INSTALL_DIR}/data/config.db || true'
...
PSVC
    ...
    # nginx ждёт uvicorn перед стартом
    mkdir -p /etc/systemd/system/nginx.service.d
    cat > /etc/systemd/system/nginx.service.d/feduk-wait.conf << NGXOV
[Service]
ExecStartPre=/bin/bash -c 'for i in \$(seq 1 20); do curl -sf --max-time 2 http://127.0.0.1:${BACKEND_PORT}/api/health &>/dev/null && exit 0; sleep 2; done; echo "uvicorn не ответил"; exit 1'
NGXOV
    ...
    run_step_soft "Запуск Xray-core" bash -c "
        systemctl enable xray-feduk
        systemctl restart xray-feduk || true
        sleep 2
    "

    run_step_soft "Запуск FEDUK Panel" bash -c "
        systemctl enable feduk
        systemctl restart feduk || true
    "

    print_ok "Сервисы: redis → xray-feduk → feduk"
    progress_bar
}
...
init_admin_db() {
    ...
    local ready=false
    while [[ $i -lt 30 ]]; do
        if curl -sk --max-time 2 "http://127.0.0.1:${BACKEND_PORT}/api/health" &>/dev/null; then
            ready=true
            break
        fi
        sleep 1
...
/opt/feduk/logs/*.log {
    ...
    rotate 14
    compress
    # ── Logrotate: добавляем логи uvicorn ──────────────────────
    cat > /etc/logrotate.d/feduk << 'EOF'
/opt/feduk/logs/*.log {
    daily
    missingok
...
EOF
    ...
    # ── Systemd timer для certbot (вместо cron) ────────────────
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ -n "$DOMAIN" ]]; then
        cat > /etc/systemd/system/certbot-feduk.service << 'CERTSVC'
[Unit]
Description=Certbot renew for FEDUK Panel
After=network-online.target

[Service]
Type=oneshot
ExecStart=/bin/bash -c 'systemctl stop nginx; certbot renew --quiet --post-hook "systemctl start nginx"'
CERTSVC

        cat > /etc/systemd/system/certbot-feduk.timer << 'CERTTMR'
[Unit]
...
CERTTMR
        systemctl enable --now certbot-feduk.timer >> "$LOG_FILE" 2>&1 || true
        print_ok "Systemd timer для certbot настроен"
    fi

    # ── Cron: бэкап + geo-данные ──────────────────────────────
...
CRONEOF
    ...
    # ── Проверяем секретный ключ JWT ──────────────────────────
    if grep -q "feduk-secret-change-me!" "${CONFIG_DIR}/config.yml" 2>/dev/null; then
        local new_secret
        new_secret=$(openssl rand -hex 32)
        sed -i "s/feduk-secret-change-me!/${new_secret}/" "${CONFIG_DIR}/config.yml"
        print_ok "JWT secret key заменён на безопасный"
        log_ok "JWT secret key regenerated"
    fi
...
# ─────────────────────────────────────────────
#  ШАГ 16: HEALTH CHECK + АВТО-ДИАГНОСТИКА
# ─────────────────────────────────────────────
health_check() {
    print_step "Проверка и запуск панели"
    ...
    # ── Функция диагностики и самолечения ─────────────────────
    _heal_services() {
        local attempt=$1
        print_info "Диагностика (попытка ${attempt}/5)..."

        # Redis
        if ! systemctl is-active --quiet redis-server 2>/dev/null; then
            print_warn "Redis не запущен — рестарт..."
            systemctl restart redis-server >> "$LOG_FILE" 2>&1 || true
            sleep 2
        fi

        # Uvicorn panel
        if ! systemctl is-active --quiet feduk 2>/dev/null; then
            print_warn "feduk не запущен — рестарт..."
            systemctl restart feduk >> "$LOG_FILE" 2>&1 || true
            sleep 5
        fi

        # Проверяем uvicorn изнутри
        if ! curl -sf --max-time 5 "http://127.0.0.1:${BACKEND_PORT}/api/health" &>/dev/null; then
            print_warn "uvicorn не отвечает — смотрим логи и рестарт..."
            tail -20 "${INSTALL_DIR}/logs/panel-error.log" >> "$LOG_FILE" 2>&1 || true
            systemctl restart feduk >> "$LOG_FILE" 2>&1 || true
            sleep 8
        fi

        # UFW порт
        if ! ufw status 2>/dev/null | grep -q "${PANEL_PORT}"; then
            print_warn "Порт ${PANEL_PORT} не открыт в UFW — открываем..."
            ufw allow "${PANEL_PORT}/tcp" comment 'FEDUK Panel' >> "$LOG_FILE" 2>&1 || true
        fi

        # Nginx порт
        if ! ss -tlnp 2>/dev/null | grep -qE ":${PANEL_PORT}[[:space:]]|:${PANEL_PORT}$"; then
            print_warn "Nginx не слушает порт ${PANEL_PORT} — рестарт..."
            # Убиваем всё что занимает порт
            fuser -k "${PANEL_PORT}/tcp" >> "$LOG_FILE" 2>&1 || true
            fuser -k "${HTTP_PORT}/tcp"  >> "$LOG_FILE" 2>&1 || true
            sleep 1
            # Проверяем конфиг
            if ! nginx -t >> "$LOG_FILE" 2>&1; then
                print_warn "Nginx конфиг сломан — пересоздаём..."
                configure_nginx
            else
        ...
        if [[ ! -f "${INSTALL_DIR}/certs/cert.pem" ]] || [[ ! -f "${INSTALL_DIR}/certs/key.pem" ]]; then
            print_warn "Сертификаты не найдены — перегенерируем..."
            _self_signed
        fi
    }
    ...
    for attempt in $(seq 1 $max_attempts); do
        start_spinner "Проверка доступности панели (попытка ${attempt}/${max_attempts})..."
        sleep 3

        # Внутренняя проверка API
        if curl -sk --max-time 10 "https://127.0.0.1:${PANEL_PORT}/api/health" &>/dev/null; then
            stop_spinner
            # Внешняя проверка по реальному IP/домену
            if curl -sk --max-time 15 "${panel_url}/api/health" &>/dev/null; then
                stop_spinner
                panel_ok=true
                break
            else
                stop_spinner
                print_warn "Внутри OK, снаружи недоступно — проверяем UFW..."
        ...
        if [[ $attempt -lt $max_attempts ]]; then
            _heal_services "$attempt"
        fi
    done

    # ── Итог ──────────────────────────────────────────────────
    echo
    for svc in redis-server xray-feduk feduk nginx; do
    ...
    if [[ "$panel_ok" == true ]]; then
        print_ok "Панель доступна: ${panel_url} ✓"
        log_ok "Panel accessible at ${panel_url}"
    else
        print_warn "Панель не ответила после ${max_attempts} попыток"
        print_warn "URL: ${panel_url}"
        echo
        echo -e "  ${YELLOW}Диагностика:${R}"
        echo -e "  ${DIM}  journalctl -u feduk -n 30${R}"
        echo -e "  ${DIM}  journalctl -u nginx -n 20${R}"
        echo -e "  ${DIM}  cat ${INSTALL_DIR}/logs/panel-error.log${R}"
        echo -e "  ${DIM}  nginx -t${R}"
        echo -e "  ${DIM}  ufw status${R}"
        log_error "Panel not accessible at ${panel_url} after ${max_attempts} attempts"
    fi
