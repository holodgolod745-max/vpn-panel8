...
NGINXEOF
    ...
    # Снимаем процессы на портах 80/443 перед запуском
    fuser -k 80/tcp  >> "$LOG_FILE" 2>&1 || true
    fuser -k 443/tcp >> "$LOG_FILE" 2>&1 || true
    sleep 1
    ...
    if systemctl restart nginx >> "$LOG_FILE" 2>&1; then
        ...
        sleep 3
        systemctl restart nginx >> "$LOG_FILE" 2>&1 || print_warn "Nginx: journalctl -u nginx для диагностики"
        print_ok "Nginx настроен (v${ngx_ver})"
    fi
    progress_bar
}
