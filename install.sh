...
# ─────────────────────────────────────────────
interactive_setup() {
    # Disable strict mode during interactive input — set -e kills the script
    # when read returns non-zero (e.g. user presses Enter on empty input)
    set +euo pipefail
    local OLD_IFS="$IFS"
    IFS=$' \t\n'
...
setup_ssl() {
    ...
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ -n "$DOMAIN" ]]; then
        ...
        run_with_spinner "Obtaining Let's Encrypt certificate for ${DOMAIN}" \
            bash -c "certbot certonly --standalone --non-interactive --agree-tos \
                --email admin@${DOMAIN} -d ${DOMAIN} 2>&1"
        ...
        # Auto-renew
        (crontab -l 2>/dev/null; echo "0 2 * * * certbot renew --quiet && systemctl reload nginx") \
