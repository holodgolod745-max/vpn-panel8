...
interactive_setup() {
    ...
    if [[ "$DOMAIN" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        print_warn "IP entered — Let's Encrypt requires a real domain. Switching to self-signed SSL."
        DOMAIN=""
    fi
    ...
    # 16 ASCII alphanum chars — safe for bcrypt (max 72 bytes)
...
setup_ssl() {
    ...
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ "$DOMAIN" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        print_warn "DOMAIN is an IP — switching to self-signed"
        USE_LETSENCRYPT=false; DOMAIN=""
    fi
    ...
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ -n "$DOMAIN" ]]; then
        run_with_spinner "Installing Certbot" \
            bash -c "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq certbot 2>/dev/null"
        ...
        if certbot certonly --standalone --non-interactive --agree-tos \
            ...
            (crontab -l 2>/dev/null
             echo "0 2 * * * systemctl stop nginx; certbot renew --quiet; systemctl start nginx") \
                | crontab - 2>/dev/null || true
            print_ok "Let's Encrypt certificate obtained for ${DOMAIN}"
            log_info "Let's Encrypt: $DOMAIN"
        else
            print_warn "certbot failed — falling back to self-signed certificate"
            _gen_selfsigned
            print_ok "Self-signed certificate generated (fallback)"
        fi

        systemctl start nginx >> "$LOG_FILE" 2>&1 || true
    else
        run_with_spinner "Generating self-signed certificate" _gen_selfsigned
        print_ok "Self-signed certificate generated (10 years)"
        log_info "Self-signed cert: ${DOMAIN:-$SERVER_IP}"
    fi
    progress_bar
}
...
configure_nginx() {
    ...
    # Detect nginx version: http2 syntax changed in 1.25.1
    ...
    local http2_listen http2_line=""
    if [[ "$ngx_maj" -gt 1 ]] || \
       [[ "$ngx_maj" -eq 1 && "$ngx_min" -gt 25 ]] || \
       [[ "$ngx_maj" -eq 1 && "$ngx_min" -eq 25 && "${ngx_pat:-0}" -ge 1 ]]; then
...
create_main_config() {
    ...
    local admin_hash
    admin_hash=$("${INSTALL_DIR}/panel/venv/bin/python3" -c "
import bcrypt, warnings; warnings.filterwarnings('ignore')
pw = '${ADMIN_PASS}'.encode('utf-8')[:72]
print(bcrypt.hashpw(pw, bcrypt.gensalt(12)).decode())
" 2>/dev/null || python3 -c "import hashlib; print(hashlib.sha256(b'${ADMIN_PASS}').hexdigest())")
...
init_admin_db() {
    ...
    run_with_spinner "Waiting for panel to start" bash -c "sleep 6"

    local VENV_PY="${INSTALL_DIR}/panel/venv/bin/python3"

    # Install compatible versions: passlib has bcrypt compatibility issue
    # Use bcrypt directly to avoid the "__about__" AttributeError
    "${VENV_PY}" -m pip install -q "bcrypt>=4.0.0" "passlib[bcrypt]>=1.7.4" 2>/dev/null || true
    # Patch passlib bcrypt backend to work with bcrypt>=4.0
    "${VENV_PY}" -c "
import bcrypt, pathlib, re
init = pathlib.Path(bcrypt.__file__).parent / '__init__.py'
# bcrypt 4+ moved __version__ out of __about__; patch passlib handler at runtime
" 2>/dev/null || true
...
try:
    ...
    def hash_password(password: str) -> str:
        """Use bcrypt directly — avoids passlib/__about__ compat issue."""
    ...
    if not existing:
        u = User(username="${ADMIN_USER}", password_hash=hashed, is_active=True)
        db.add(u)
        db.commit()
    ...
    # Verify it works
    check = db.query(User).filter(User.username == "${ADMIN_USER}").first()
    if verify_password("${ADMIN_PASS}", check.password_hash):
        print("Password verified OK")
    else:
        print("WARNING: password verify failed!", file=sys.stderr)
        sys.exit(1)
...
INITPY
    ...
    if [[ $? -ne 0 ]]; then
        print_warn "DB init error — check ${LOG_FILE}"
    else
        print_ok "Admin account ready (user: ${ADMIN_USER}  pass: ${ADMIN_PASS})"
    fi
