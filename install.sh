...
# ─────────────────────────────────────────────
interactive_setup() {
    # set -e kills script when read gets empty Enter (returns 1)
    set +euo pipefail
    local OLD_IFS="$IFS"
    IFS=$' \t\n'
    ...
    # Reject raw IPs — Let's Encrypt only works with real domain names
...
setup_ssl() {
    ...
    if [[ "$USE_LETSENCRYPT" == true ]] && [[ -n "$DOMAIN" ]]; then
        ...
        # Stop nginx — certbot --standalone needs port 80
...
configure_nginx() {
    ...
    # Detect nginx version: http2 directive syntax changed in 1.25.1
    ...
    log_info "Nginx ${ngx_ver}: listen='${http2_listen}' http2_line='${http2_line}'"
...
init_admin_db() {
    ...
    # Wait for panel service to create the database
    run_with_spinner "Waiting for panel to start" bash -c "sleep 6"

    local VENV_PY="${INSTALL_DIR}/panel/venv/bin/python3"

    # Use venv python directly — system python3 doesn't have passlib/bcrypt
    "${VENV_PY}" - <<INITPY
import sys, os
...
INITPY
    ...
    if [[ $? -ne 0 ]]; then
        print_warn "DB init had errors — check log. Trying to reset password via API..."
        # Fallback: rewrite config.yml hash so panel uses YAML auth
        local admin_hash
        admin_hash=$("${VENV_PY}" -c \
            "from passlib.context import CryptContext; print(CryptContext(schemes=['bcrypt']).hash('${ADMIN_PASS}'))" 2>/dev/null)
        if [[ -n "$admin_hash" ]]; then
            sed -i "s|^admin_password_hash:.*|admin_password_hash: "${admin_hash}"|" "${CONFIG_DIR}/config.yml"
            print_ok "Password hash updated in config.yml"
        fi
    fi

    print_ok "Admin account initialised (user: ${ADMIN_USER}  pass: ${ADMIN_PASS})"
